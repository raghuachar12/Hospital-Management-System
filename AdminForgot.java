...
public class AdminForgot {
	...
	public JFrame frameAdminForgot;
	...
	private void initialize() {
		frameAdminForgot = new JFrame();
		frameAdminForgot.setTitle("Create new Password");
		frameAdminForgot.setBounds(100, 100, 752, 588);
		frameAdminForgot.setLocationRelativeTo(null);
		frameAdminForgot.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frameAdminForgot.getContentPane().setLayout(null);
		
		JLabel lblNewLabel = new JLabel("ADMIN ID :");
		lblNewLabel.setFont(new Font("Tahoma", Font.PLAIN, 25));
		lblNewLabel.setBounds(58, 145, 232, 47);
		frameAdminForgot.getContentPane().add(lblNewLabel);
		
		JLabel lblPhoneNo = new JLabel("PHONE NO. :");
		lblPhoneNo.setFont(new Font("Tahoma", Font.PLAIN, 25));
		lblPhoneNo.setBounds(58, 238, 232, 47);
		frameAdminForgot.getContentPane().add(lblPhoneNo);
		
		textA_id = new JTextField();
		textA_id.setBounds(348, 147, 200, 56);
		frameAdminForgot.getContentPane().add(textA_id);
		textA_id.setColumns(10);
		
		textP_no = new JTextField();
		textP_no.setColumns(10);
		textP_no.setBounds(348, 240, 200, 56);
		frameAdminForgot.getContentPane().add(textP_no);
		
		JLabel lblEnterNewPassword = new JLabel("Enter New Password :");
		lblEnterNewPassword.setFont(new Font("Tahoma", Font.PLAIN, 25));
		lblEnterNewPassword.setBounds(58, 348, 249, 31);
		frameAdminForgot.getContentPane().add(lblEnterNewPassword);
		
		passwordField = new JPasswordField();
		passwordField.setBounds(348, 348, 200, 56);
		frameAdminForgot.getContentPane().add(passwordField);
		...
		frameAdminForgot.getContentPane().add(btnShow);
		...
		frameAdminForgot.getContentPane().add(btnUpdate);
	}
	private void btnNewButtonactionPerformed(ActionEvent evt)
	{
		String A_id=textA_id.getText();
		...
		try
		{
			...
			String update="update ADMINISTRATION set PASSWORD=? where A_ID=? and PH_NO=?";
