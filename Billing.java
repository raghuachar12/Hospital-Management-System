...
public class Billing {
	...
	private void btnSaveToDataBaseActionPerformed(ActionEvent evt)
	{
		...
		try {
			Connect.getup();
			con=Connect.Connecti();
			String sql="insert into BILLING(BILL_NUMBER,P_ID,P_NAME,P_TYPE,DOCTOR_CHARGES,LAB_CHARGES,OTHER_CHARGES,TOTAL)values(?,?,?,?,?,?,?,?)";
	...
	private void btnInSaveActionPerformed(ActionEvent evt)
	{
		...
		try
		{
			Connect.getup();
			con=Connect.Connecti();
			String sql="insert into BILLING(BILL_NUMBER,P_ID,P_NAME,P_TYPE,DOCTOR_CHARGES,LAB_CHARGES,WARD_CHARGES,NO_OF_DAYS,OTHER_CHARGES,TOTAL)values(?,?,?,?,?,?,?,?,?,?)";
			p=con.prepareStatement(sql);
